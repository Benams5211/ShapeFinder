...
function spawnInteractors(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let obj;
    if (choice === 'circle') {
      obj = new ScoreDownCircle(width/2, height/2, 40, null, opts);
    } else if (choice === 'rect') {
      obj = new ScoreDownRect(width/2, height/2, 40, 80, null, 8, opts);
    } else {
      obj = new ScoreDownTri(width/2, height/2, 80, null,
        // added random rotation
        { ...opts, angle: random(TWO_PI) }
    );}
