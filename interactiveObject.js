...
function spawnWinShape() {
  // movement config same as before
  const movement = {
    enabled: true,
    lerpStrength: 0.1,
    velocityLimit: 4,
  ...
  // adjust win shape size based on difficulty
  let size;
  if (difficulty === "easy") {
    size = 60;    // bigger target
  } else if (difficulty === "medium") {
    size = 40;    // balanced
  } else if (difficulty === "hard") {
    size = 25;    // smaller target
  }

  // pick a random win shape type, like before
  ...
  if (makeCircle) {
    ...
    obj = new WinCircle(
      ...
      [255, 255, 255],        // color same as original
...
function spawnInteractors(count) {
  ...
  if (difficulty === "easy") {
    ...
    numObjects = 30;  // fewer objects
  } else if (difficulty === "medium") {
    size = 40;        // medium size & amount
    numObjects = 60;  
  } else if (difficulty === "hard") {
    size = 25;        // smaller shapes
    numObjects = 100; // more objects
  }

  // put a hard cap similar to the old code
  if (interactors.length >= numObjects) return;
  ...
  for (let i = 0; i < numObjects; i++) {
    ...
    let obj;
    if (makeCircle) {
      // roughly the same visual size as Shape, scaled by difficulty
  ...
  // always add a win shape at the end
  spawnWinShape();
}
