    ...
    events.start(CURTAINS_EVENT, totalDuration, {
      ...
      onUpdate: (timeLeft, isFrontPass) => {
        ...
        if (phase === 'closing') {
          ...
          const t = constrain((elapsed - halfDuration - pauseDuration) / halfDuration, 0, 1);
          curtainProgress = 1 - t;
        }

        // Only draw in front-pass
        ...
        noStroke();
        fill(139, 0, 0);
        rect(0, 0, w, height);         // left curtain
        rect(width - w, 0, w, height); // right curtain
        pop();
      },

      onEnd: () => {
        console.log("Curtains end");
      }
    });
  }
