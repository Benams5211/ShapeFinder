...
function spawnMenuShape() {
  ...
  if (random() < 0.50) {
      mods.push(new FigureSkateModifier({
        ...
        joinChance: 0.005,
        strength: 0.10,
        types: ['circle','orbit','figure8','line','triangle','orbitTriangle','square','orbitSquare'],
        minGapFrames: 300,
      }));
  }
  const opts = {
...
function keyPressed() {
  if (key === '1') formationDirector.start(interactors, { type: 'circle' });
  if (key === '2') formationDirector.start(interactors, { type: 'orbit' });
  if (key === '3') formationDirector.start(interactors, { type: 'figure8' });
  if (key === '4') formationDirector.start(interactors, { type: 'square' });
  if (key === '5') formationDirector.start(interactors, { type: 'orbitSquare' });
  if (key === '6') formationDirector.start(interactors, { type: 'triangle' });
  if (key === '7') formationDirector.start(interactors, { type: 'orbitTriangle' });
